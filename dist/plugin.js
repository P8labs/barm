...
import { createResource, deleteResource, getResource, listResource, updateResource, } from "./operations/index.js";
export function resourceManager(options) {
    const endpoints = {};
    for (const [name, resource] of Object.entries(options.resources)) {
        ...
        for (const operation of Object.keys(endpointDefinitions)) {
            if (!access[operation]) {
                continue;
            }
            const definition = endpointDefinitions[operation];
            endpoints[`${name}${capitalize(operation)}`] = createAuthEndpoint(definition.path(name), {
