...
usage() {
  cat <<'EOF'
Usage:
  scripts/release.sh <major|minor|patch|x.y.z> [--push]
...
What it does:
  ...
  2) Creates commit: chore(sdk): release @P8labs/barm v<version>
...
git -C "$REPO_ROOT" commit -m "chore(sdk): release @P8labs/barm v$NEW_VERSION"

git -C "$REPO_ROOT" tag -a "$TAG" -m "Release @P8labs/barm v$NEW_VERSION"
