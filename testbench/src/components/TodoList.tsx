...
function TodoItem({
  ...
  const bgColor = todo.completed
    ...
    : "bg-transparent hover:bg-blue-50 hover:bg-opacity-30";
        ...
        <div className="flex items-center gap-2 flex-1 min-w-0">
          ...
          {isEditing ? (
            ...
            <span
              title={todo.title}
              onClick={() => onEdit(todo)}
              className={`flex-1 truncate text-lg font-medium min-w-0 cursor-pointer hover:bg-gray-50 hover:px-2 hover:py-1 hover:rounded transition-all duration-150 ${
...
export default function TodoList({ userId }: { userId: string }) {
  ...
  const fetchTodos = async () => {
    try {
      const { data: todos, error } =
        await authClient.resourceManager.todo.list();

      if (error) {
        throw new Error(
          `Failed to fetch todos: ${error.message || error.statusText}`,
        );
      }

      const sortedTodos = todos?.data || [];
  ...
  const deleteTodo = async (id: string) => {
    ...
    setIsSyncing(true);
    try {
      const { error } = await authClient.resourceManager.todo.delete({ id });
    ...
    try {
      ...
      await Promise.all(
        completedTodos.map((todo) =>
          authClient.resourceManager.todo.delete({ id: todo.id }),
        ),
      );
      setLastSyncTime(new Date());
    } catch (error) {
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    try {
      ...
      const allOrderUpdates = newTodosState.map((todo) => ({
        id: todo.id,
        order: newTodosState.findIndex((t) => t.id === todo.id) * 10,
      }));

      console.log(
        `Sending ${allOrderUpdates.length} order updates to database`,
      );
        ...
        <div className="max-w-2xl mx-auto">
          ...
          <div className="bg-white bg-opacity-90 backdrop-blur-sm shadow-2xl rounded-xl border border-amber-200 relative">
            ...
            <div className="absolute inset-0 opacity-10 pointer-events-none">
              <div
                className="h-full bg-gradient-to-b from-transparent via-blue-200 to-transparent bg-[length:100%_24px] bg-repeat-y"
                style={{
                  backgroundImage:
                ...
                {activeExpanded && (
                  <div>
                    {activeTodos.length === 0 ? (
                      <p className="text-gray-400 text-center py-8 pl-6 font-medium">
                        All caught up! 🎉
                      </p>
                    ) : (
                      <SortableContext
